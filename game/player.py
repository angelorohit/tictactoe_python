...
class Player:
    ...
    def take_turn(self, game_board_size: int) -> (int, int, Optional[str]):
        turn_input = input('Enter your position (Example, A1 or C2): ')
        return self.validate_turn_input(turn_input, game_board_size)
    ...
    def validate_turn_input(self, turn_input: str, game_board_size: int) -> (int, int, Optional[str]):
        ...
        if row_pos >= game_board_size:
            return 0, 0, f'You must enter a row value that is within the playing board size of 1 to {game_board_size}'

        upper_a_ascii = ord('A')
        col_pos = ord(col.upper()) - upper_a_ascii

        if col_pos >= game_board_size:
            return 0, 0, f'You must enter a column value that is within the playing board size of A to {chr(upper_a_ascii + game_board_size - 1)}'
